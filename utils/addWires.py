...
def addLines(data, ignore_calib=False, force_connect_towers=None):
    TowerIDS = tower.util.tower_ids_json(data)
    if len(TowerIDS) <= 1: return data
    tower_lines_pairing = tower.util.sort_tower_ids_by_distance(data, TowerIDS, force_connect_towers)

    pois = tower.util.check_matching_poi(data, TowerIDS)  # noqa: F841

    for tID1, tID2 in list(tower_lines_pairing):
        pois_count = min(tower.util.get_numberof_pois_from_tower(data, tID1),
                         tower.util.get_numberof_pois_from_tower(data, tID2))
...
def calculate_cost_matrix(data, tID1, tID2):
    ...
    cost_matrix = np.asarray(
        [[tower.util.GeoToCartesianMeters(poi1, poi2) for poi2 in pois_tower2] for poi1 in pois_tower1])
