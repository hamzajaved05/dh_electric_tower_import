...
def add_names(json_dict: dict, xml_dict: dict) -> dict:
    ...
    if towers_in_json != towers_in_xml:
        warnings.warn(f"Incorrect matching for number of towers -> xml: {towers_in_xml} vs json -> {towers_in_xml}")
    ...
    for feature in json_dict["features"]:
        if feature["properties"]["type"] == "electric_pole":
            ...
            distances = np.linalg.norm([GeoToCartesianMeters(json_coords, xml_coord) for xml_coord in xml_coordinates],
                                       axis=1)
    ...
    print(
        f"Matching towers {len(tower_association_deltas)}\nmax delta {np.max(tower_association_deltas)}\nskipped towers from json {skipped_towers}\nxml towers {towers_in_xml}\njson towers {towers_in_json}")
    return json_dict


if __name__ == "__main__":
    file_path = '/mnt/00A03D3C0BCCF8D8/Data/PowerLinesData/20230508SwissGrid/testLine/TR1670_Riddes-Avise_Valpeline_IT/TR1670_Riddes-Avise_Valpeline_IT.json'
